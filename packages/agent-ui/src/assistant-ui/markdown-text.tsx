...
const CodeHeader: FC<CodeHeaderProps> = ({ language, code }) => {
    ...
    return (
        <div className="aui-code-header-root mt-2.5 flex items-center justify-between rounded-t-lg border border-border/50 border-b-0 bg-muted/50 px-3 py-1.5 text-xs">
            <span className="aui-code-header-language font-medium text-muted-foreground lowercase leading-10">
                {language}
            </span>
...
const defaultComponents = memoizeMarkdownComponents({
    ...
    p: ({ className, ...props }) => (
        <p
            className={cn(
                "aui-md-p my-2.5 leading-relaxed first:mt-0 last:mb-0",
                className,
            )}
            {...props}
        />
    ...
    ol: ({ className, ...props }) => (
        <ol
            className={cn(
                "aui-md-ol my-2 ms-4 list-decimal marker:text-muted-foreground [&>li]:mt-4",
                className,
            )}
            {...props}
        />
